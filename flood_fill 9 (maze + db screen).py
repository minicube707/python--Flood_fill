...
def draw_screen_bot (win,  width, rows, list_short_path):
    gap = width // rows   
    new_list_short_path = []
...
#Draw the window
def draw (win, rows, width, start, end, pathfinder, set_wall, set_path, set_explore_path, list_short_path):

    #Initialisation
    WIN.fill(WHITE)
    drawn_global_vision(win, width, rows, start, end, pathfinder, set_wall, set_path, set_explore_path)
    draw_screen_bot (win,  width, rows, list_short_path)
...
#Clean the window
def clean_window(set_path, set_explore_path, list_short_path):
    set_path.clear()
    set_explore_path.clear()
    list_short_path.clear()

    return set_path, set_explore_path, list_short_path
...
def algorithm(win, grid, width, rows, run, stop, start, end, set_wall, set_explore_path):
    histroy_node = []
    ...
    while grid[current_node] != 1 and run and not stop:
        ...
        grid, run, stop = scann_neigbhors(grid, current_node, rows, run, stop, set_wall)
        ...
        draw(win, rows, width, start, end, current_node, set_wall, {}, set_explore_path, list_short_path)
...
def algorithm_path (win, grid, width, rows, run, stop, start, end, set_wall, set_path):
    ...
    draw(win, rows, width, start, end, None, set_wall, set_path, {}, list_short_path)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_g:
                    set_path, set_explore_path, list_short_path = clean_window(set_path, set_explore_path, list_short_path)
                ...
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    while not two_time and run and not stop:
                        ...
                        run, stop = algorithm(win, go_grid, width, rows, run, stop, start, end, set_wall, set_explore_path)
                        ...
                        run, stop = algorithm(win, back_grid, width, rows, run, stop, end, start, set_wall, set_explore_path)
                    ...
                    list_short_path = algorithm_path(win, go_grid, width, rows, run, stop, start, end, set_wall, set_path)
