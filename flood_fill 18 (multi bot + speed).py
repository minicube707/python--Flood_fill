...
def algorithm_path (win, grid, width, rows, run, stop, start, end, set_wall):
    current_node = start
    set_path = set()
    show = True
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start and end:
                    stop = False
                    nb_bot = int(input("How many bot do you want ? "))
                    print(nb_bot, "bot(s)")
                    ...
                    for i in range(nb_bot):
                        ...
                        if i%2 == 0:
                            list_horizontal.append(True)
                        else:
                            list_horizontal.append(False)
                    ...
                    while run and not stop and not finished:
                        ...
                        for i in range(nb_bot):
                            ...
                            if (current_time - start_time) >=  (i * time_bt_bot):
                                ...
                                run, stop, wall_grid, list_current_start[i], list_go[i], finished, list_save_grid[i] = bot(wall_grid, rows, run, stop, start, end, list_current_start[i], set_wall, list_go[i], list_save_grid[i], list_horizontal[i])
                    ...
                    map_path = make_grid(wall_grid, end, rows)
                    set_path = algorithm_path(win, map_path, width, rows, run, stop, start, end, set_wall)
