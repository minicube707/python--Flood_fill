...
def bot(wall_grid, rows, run, stop, start, end, current_node, set_wall, go, save_grid, horizontal): 
    ...
    return run, stop, new_wall_grid, current_node, go, save_grid
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    #Bot1
                    current_node = start
                    save_grid = np.zeros((rows, rows), int)
                    go = True
                    
                    #Verify two time than the path is clear
                    run, stop, wall_grid, current_node, go, save_grid = bot(wall_grid, rows, run, stop, start, end, current_node, set_wall, go, save_grid, True)
