...
#Show a grid who represente the difference between the fisrt grid and the last grid
def draw_grid_evolution(grid, width, rows, end, set_wall):
    gap = width // rows

    #Graphic on the evolution of the grid
    plt.figure()

    #Draw the grid
    for i in range(rows + 1):
        plt.plot((0, width), (i * gap, i * gap), color = "k", linewidth=0.5)

    for j in range(rows + 1):
        plt.plot((j * gap, j * gap), (0, width), color = "k", linewidth=0.5)

    #Make the grid of difference between the fisrt gride and the last gride
    original_grid = np.zeros((rows, rows), int)
    original_grid = make_grid(original_grid, end, rows)
    diff_grid = np.abs(grid - original_grid)
    
    # Étape 1: Trouver les valeurs minimale et maximale
    diff_grid[grid <= 0] = 0
    min_val = np.min(diff_grid)
    max_val = np.max(diff_grid)

    # Étape 2: Normaliser les valeurs pour qu'elles soient entre 0 et 1
    if max_val != min_val:
        normalized_arr = (diff_grid - min_val) / (max_val - min_val)
    else:
        normalized_arr = np.zeros((rows, rows), int)

    # Étape 3: Multiplier par 255 pour obtenir des valeurs entre 0 et 255
    scaled_arr = (1 -normalized_arr )

    #Make the colour grid
    arr_tuples = np.array([], int)
    for row, line in enumerate (scaled_arr):
        for col, val in enumerate (line):
            
            if (row, col) in set_wall:
                arr_tuples = np.append(arr_tuples, (0, 0, 0))
            else:
                arr_tuples = np.append(arr_tuples, (val, val, 1))

    #Put the array in right shape and orientation
    arr_tuples = arr_tuples.reshape((rows, rows, 3))
    arr_tuples = np.flipud(arr_tuples)

    #Draw the colour of the grid
    for col, line in enumerate (arr_tuples):
        for row, colour in enumerate (line):
            plt.fill((row*gap, row*gap, (row+1)*gap, (row+1)*gap), (col*gap, (col+1)*gap, (col+1)*gap, col*gap), color = colour)

    plt.title("Grid of difference between the fisrt grid and the last grid")
    plt.axis("off")



    #Graphic of the end grid
    plt.figure()
    ...
    max_val = np.max(grid)
    if max_val != min_val:
        normalized_arr = (grid - min_val) / (max_val - min_val)
    else:
        normalized_arr = np.zeros((rows, rows), int)
    ...
    for row, line in enumerate (scaled_arr):
        for col, val in enumerate (line):
            ...
            if (row, col) in set_wall:
                ...
                arr_tuples = np.append(arr_tuples, (1, val, val))
    ...
    plt.title("Graphic of the distance to the end point")
    plt.axis("off")
    plt.show()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    draw_grid_evolution(go_grid, width, rows, end, set_wall)
